...
(function() {
  ...
  setTimeout(function() {
    ...
    var P = {
      ...
      review: { url: '/store/p/virtual-garden-review', title: 'Virtual Garden Review', line: 'A written assessment of up to 10 photos and 3 questions, delivered within 5 business days.', price: '$49', service: true },
    ...
    var RULES = [
      [/^troubleshooting-mulberry-problems$/, 'review'],
...
(function() {
  ...
  function inject() {
    if (slug === 'troubleshooting-mulberry-problems') return;
...
(function () {
  ...
  function calendar() {
    ...
    var desertOption=q('option[value="desert"]',zone);if(desertOption)desertOption.textContent='Low desert valleys';
    ...
    function render(){
      ...
      if(z==='desert'){var zoneName=q('#zd-name');if(zoneName)zoneName.textContent='Low desert valleys';}
      all('.pc-nt[data-s="garlic"][data-a="ds"]',tags).forEach(function(b){b.textContent='Garlic: plant cloves';b.setAttribute('aria-label','Garlic: plant cloves outdoors');});
      all('.pc-nt[data-s="potatoes"][data-a="ds"]',tags).forEach(function(b){b.textContent='Potatoes: plant seed potatoes';b.setAttribute('aria-label','Potatoes: plant seed potatoes outdoors');});
      all('tr[data-s="garlic"] [title],tr[data-s="garlic"] [aria-label]',tbody).forEach(function(node){['title','aria-label'].forEach(function(a){var v=node.getAttribute(a);if(v&&/direct sow/i.test(v))node.setAttribute(a,v.replace(/direct sow/ig,'Plant cloves outdoors'));});});
      all('tr[data-s="potatoes"] [title],tr[data-s="potatoes"] [aria-label]',tbody).forEach(function(node){['title','aria-label'].forEach(function(a){var v=node.getAttribute(a);if(v&&/direct sow/i.test(v))node.setAttribute(a,v.replace(/direct sow/ig,'Plant seed potatoes outdoors'));});});
      ...
      if(tags.querySelector(':scope > .pc-nt')){
        ...
        buttons.forEach(function(b){var c=DATA.crops.find(function(c){return c.s===b.dataset.s;});if(c&&timingReview(c,z))return;var key=Object.keys(colors).find(function(k){return colors[k]===b.dataset.a;});if(key){b.style.removeProperty('display');groups[key].appendChild(b);}});
      ...
      var count=0;all('tr[data-s]',tbody).forEach(function(row){var c=DATA.crops.find(function(c){return c.s===row.dataset.s;});var yes=!!c && !timingReview(c,z) && matches(actions(c,z,mn));row.hidden=!yes;if(yes)count++;});
  ...
  function profile(){
    ...
    root.innerHTML='<h2 id="ah-garden-title">Show me my garden</h2><p>Start with your California location, light, and growing space. Save useful planting tools and resources for your next visit.</p><form id="ah-garden-form"><div class="ah-garden-fields"><label>California ZIP code<input name="zip" inputmode="numeric" autocomplete="postal-code" pattern="[0-9]{5}" maxlength="5" required></label><label>Sun<select name="sun"><option value="full">Full sun</option><option value="partial">Partial sun</option><option value="shade">Shade</option></select></label><label>Growing space<select name="environment"><option value="ground">In the ground</option><option value="raised_beds">Raised beds</option><option value="containers">Containers</option><option value="mixed">A mix of spaces</option></select></label><label>Main interest<select name="interest"><option value="vegetables">Vegetables</option><option value="fruit">Fruit</option><option value="flowers">Flowers</option><option value="natives">California natives</option><option value="herbs">Herbs</option></select></label><label>Calendar region<select name="region"><option value="">Suggest from my ZIP</option><option value="coastal">Coastal</option><option value="inland_valley">Inland Valley</option><option value="mountain">Mountain</option><option value="desert">Low desert valleys</option></select><small>Imperial and Coachella are low desert. High-desert gardens may need timing beyond these four regions.</small></label></div><button type="submit">Show my garden</button><small>Your profile stays in this browser. No account or email is required.</small><p id="ah-garden-message" role="status"></p></form><div id="ah-garden-result" hidden></div>';
    ...
    function show(p,persisted){
      ...
      region.addEventListener('change',function(){p.zone=region.value;p.regionConfirmed=true;form.elements.region.value=p.zone;var savedOK=false;try{localStorage.setItem('ah_garden_profile_v1',JSON.stringify(p));savedOK=true;}catch(e){}show(p,savedOK);event('garden_profile_updated',{climate_zone:p.zone});});
      ...
      if(cat&&p.sun==='full'){
        ...
        if(crops.length){var ul=el('ul');crops.forEach(function(c){ul.appendChild(el('li',c.n+': '+actions(c,p.zone,currentMonth()).filter(function(k){return k==='d'||k==='x';}).map(function(k){return actionName(c,k);}).join(' or ')));});result.appendChild(ul);}else result.appendChild(el('p','No outdoor planting window is listed for this interest and region this month. Use the calendar to plan ahead.'));
      }
  ...
  function articleLinks(){
    ...
    },{"/learn/watermelon-growth-stages": ["After the melon harvest, ", "/planting-calendar", "check the planting calendar for your next California crop", ". Choose your zone and month, then use Plant now to separate sowing and transplanting from harvest windows."], "/learn/why-your-peppers-wont-turn-red": ["For the local weather context, ", "/garden-conditions", "check Santa Cruz County garden conditions", " alongside the ripening checks above."], "/learn/vegetables-partial-shade-california": ["Once you know how much sun reaches the bed, ", "/planting-calendar#my-garden", "set up My Garden with your ZIP and sun exposure", ". Your choices stay in this browser and help you find relevant planting guidance."], "/learn/crops-windy-coastal-gardens": ["Before planning garden work, ", "/garden-conditions", "check Santa Cruz County garden conditions", " and compare them with the exposure in your own garden."], "/learn/santa-cruz-microclimates": ["Put your location into practice: ", "/planting-calendar#my-garden", "set up My Garden with your ZIP code", " and check the suggested region before using the planting calendar. Your garden choices stay in this browser."], "/learn/pumpkin-growth-stages": ["Planning the bed after your pumpkins? ", "/planting-calendar", "Check the planting calendar for your California zone", " and choose Plant now for direct-sowing and transplanting windows."], "/learn/tomato-growth-stages": ["Choosing a variety for next season? ", "/tomato-quiz", "Try the Tomato Finder", " to narrow the options before making your seed list."], "/learn/growing-tomatoes-santa-cruz": ["Before choosing a variety, ", "/tomato-quiz", "try the Tomato Finder", " as a starting point for your California garden."]})[path];if(!copy||q(".ah-next-step"))return;
    var body=q('.blog-item-content-wrapper')||q('[data-content-field="body"]')||q('.entry-content');if(!body)return;
    var headings=all('h2,h3',body).filter(function(h){return !h.closest('aside,.ah-graphic,.ah-prod,.ah-relwrap');});
    var expected={"/learn/watermelon-growth-stages": "What Is the Watermelon Growth Stage Timeline, Day by Day?", "/learn/why-your-peppers-wont-turn-red": "Which Peppers Ripen Most Reliably in Our Climate?", "/learn/vegetables-partial-shade-california": "1. Lettuce (3-4 Hours of Sun)", "/learn/crops-windy-coastal-gardens": "1. Kale", "/learn/santa-cruz-microclimates": "How Do I Find My Microclimate in Santa Cruz County?", "/learn/pumpkin-growth-stages": "What Is the Pumpkin Growth Stage Timeline in Days?", "/learn/tomato-growth-stages": "How Does Tomato Germination Work?", "/learn/growing-tomatoes-santa-cruz": "What Are the Best Tomato Varieties for Santa Cruz County?"};
    var head=expected[path]?headings.find(function(h){return h.textContent.trim()===expected[path];}):headings[0];
    // A later prose refresh can rename headings; preserve the answer and fall back to the FAQ boundary.
    if(!head)head=headings.find(function(h){return /frequently asked|related articles/i.test(h.textContent);});if(!head)return;
    var already=all('a[href]',body).some(function(a){return a.getAttribute('href')===copy[1]&&!a.closest('aside,.ah-prod,.ah-relwrap,.ah-product-callout,.ah-related');});if(already)return;
    var p=el('p',null,'ah-next-step'),a=el('a',copy[2]);a.href=copy[1];a.dataset.ahPlacement='growth_article';p.appendChild(document.createTextNode(copy[0]));p.appendChild(a);p.appendChild(document.createTextNode(copy[3]));head.parentNode.insertBefore(p,head);
  }
  // Existing free-guide delivery destinations, allowlisted by resource identity.
  function resourceDelivery(){
    if(path!=='/your-garden-toolkit')return;
    var resources=[{"slug":"tomato-selector","title":"Tomato Variety Selector","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Tomato-Selector.pdf"},{"slug":"seasonaltaskschecklist","title":"Seasonal Garden Tasks Checklist","url":"https://www.ambitiousharvest.com/s/AmbitiousHarvestSeasonalTasksChecklistpdf.pdf"},{"slug":"seasonalplantingcalendar","title":"Seasonal Planting Calendar: Monthly Reference","url":"https://www.ambitiousharvest.com/s/AmbitiousHarvestSeasonalPlantingCalendar.pdf"},{"slug":"microclimate-worksheet","title":"Know Your Microclimate Worksheet","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Microclimate-Worksheet.pdf"},{"slug":"garden-checklist","title":"Beginner Garden Setup Checklist","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Garden-Checklist.pdf"},{"slug":"fire-wise-gardening-guide","title":"Fire-Wise Gardening Guide","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Fire-Wise-Gardening-Guide.pdf"},{"slug":"companion-planting","title":"Companion Planting Guide","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Companion-Planting.pdf"},{"slug":"seasonal-planting-calendar","title":"Seasonal Planting Calendar","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Seasonal-Planting-Calendar.pdf"},{"slug":"seed-starting-guide","title":"Seed Starting Guide","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Seed-Starting-Guide.pdf"},{"slug":"vegetables-by-season","title":"Vegetables by Season","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Vegetables-by-Seasonpdf.pdf"},{"slug":"gopher-guide","title":"Gopher Control Guide","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Gopher-Guide.pdf"},{"slug":"water-wise-gardening-guide","title":"Water-Wise Gardening Guide","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Water-Wise-Gardening-Guidepdf.pdf"},{"slug":"garden-troubleshooting","title":"Garden Troubleshooting Guide","url":"https://www.ambitiousharvest.com/s/Ambitious-Harvest-Garden-Troubleshooting.pdf"},{"slug":"berry-growing-cheat-sheet","title":"California Berry Growing Cheat Sheet","url":"https://gaughanadrienne-gif.github.io/ah-graphics/Berry_Growing_Cheat_Sheet.pdf"},{"slug":"pepper-growing-cheat-sheet","title":"California Pepper Growing Cheat Sheet","url":"https://gaughanadrienne-gif.github.io/ah-graphics/Pepper_Growing_Cheat_Sheet.pdf"},{"slug":"native-drought-cheat-sheet","title":"California Native and Drought-Tolerant Plant Cheat Sheet","url":"https://gaughanadrienne-gif.github.io/ah-graphics/Native_Drought_Cheat_Sheet.pdf"},{"slug":"seed-starting-cheat-sheet","title":"California Seed Starting Cheat Sheet","url":"https://gaughanadrienne-gif.github.io/ah-graphics/Seed_Starting_Cheat_Sheet.pdf"},{"slug":"predator-proof-coop-checklist","title":"Predator-Proof Coop Checklist","url":"https://gaughanadrienne-gif.github.io/ah-graphics/Predator_Proof_Coop_Checklist.pdf"},{"slug":"fruit-tree-planting-guide","title":"California Fruit Tree Planting Guide","url":"https://gaughanadrienne-gif.github.io/ah-graphics/Fruit_Tree_Planting_Guide.pdf"}];
    function renderDownload(){
      var slug=new URLSearchParams(location.hash.slice(1)).get('download');
      var resource=resources.find(function(r){return r.slug===slug;}),old=q('#ah-requested-download');
      if(!resource){if(old)old.remove();return;}
      if(old&&old.dataset.resource===slug)return;if(old)old.remove();
      var main=q('main')||q('#page');if(!main)return;
      var panel=el('section',null,'ah-garden');panel.id='ah-requested-download';panel.dataset.resource=slug;panel.setAttribute('aria-labelledby','ah-download-title');
      var title=el('h2',resource.title);title.id='ah-download-title';panel.appendChild(title);
      panel.appendChild(el('p','Your guide is ready. No additional signup is needed.'));
      var link=el('a','Download PDF','ah-action');link.href=resource.url;link.target='_blank';link.rel='noopener';link.setAttribute('aria-label','Download '+resource.title+' PDF');
      link.addEventListener('click',function(e){if(e.isTrusted)event('toolkit_download_requested',{resource_slug:resource.slug,measurement_scope:'download_link_activation'});});panel.appendChild(link);
      panel.appendChild(el('small','Opens the PDF in a new tab. Save a copy to keep it handy.'));
      var next=el('p'),calendar=el('a','Check current planting windows');calendar.href='/planting-calendar';next.appendChild(calendar);panel.appendChild(next);
  ...
  function commerceTracking(){
    if(window.__ahCartReceiptTracking)return;window.__ahCartReceiptTracking=true;
    function isEntryPost(method,url){try{var u=new URL(url,location.href);return String(method).toUpperCase()==='POST'&&u.origin===location.origin&&u.pathname==='/api/commerce/shopping-cart/entries';}catch(e){return false;}}
    function receipt(status,body,requestBody){
      if(status<200||status>=300)return;
      try{
        var response=typeof body==='string'?JSON.parse(body):body,request=typeof requestBody==='string'?JSON.parse(requestBody):requestBody;
        var item=response&&response.newlyAdded,cart=response&&response.shoppingCart;
        if(!item||!cart||!request||String(item.itemId)!==String(request.itemId)||!Array.isArray(cart.entries))return;
        var entry=cart.entries.find(function(e){return String(e.itemId)===String(item.itemId);});
        var cents=Number(item.purchasePriceCents),quantity=Math.min(Number(request.quantity),Number(item.quantity));
        var currency=item.item&&item.item.price&&item.item.price.currency;
        if(!entry||!item.title||!currency||item.purchasePriceCents==null||!isFinite(cents)||cents<0||!Number.isInteger(quantity)||quantity<=0)return;
        event('add_to_cart',{currency:currency,value:Math.round(cents*quantity)/100,items:[{item_id:String(item.itemId),item_name:item.title,price:cents/100,quantity:quantity}],measurement_scope:'successful_cart_response'});
      }catch(e){/* Measurement must never interrupt commerce. */}
    }
    // Observe the response without changing the request, status, body, or native handlers.
    var open=XMLHttpRequest.prototype.open,send=XMLHttpRequest.prototype.send,requests=new WeakMap();
...
// === AH PRODUCT LIBRARY GENERATED START ===
/* AH library: additive, idempotent. Existing delivery and forms remain authoritative. */
(function(){
  'use strict';
  var config={"fragment": "<style>\n.ah-library{--ink:#1a3b2a;--paper:#f8f9f0;--link:#2e6b46;background:var(--paper);color:var(--ink);font-family:Montserrat,Arial,sans-serif;max-width:1120px;margin:48px auto;padding:clamp(22px,5vw,64px);line-height:1.65;box-sizing:border-box}\n.ah-library *{box-sizing:border-box}.ah-library h1,.ah-library h2,.ah-library h3{font-family:Fraunces,Georgia,serif;color:var(--ink);line-height:1.15;letter-spacing:-.02em;text-wrap:balance}.ah-library h1,.ah-library .library-heading{font-size:clamp(32px,5vw,56px);margin:0 0 24px}.ah-library h2{font-size:clamp(28px,3vw,38px);margin:0 0 20px}.ah-library h3{font-size:24px;margin:0 0 12px}.ah-library p{max-width:70ch;margin:0 0 18px}.ah-library a{color:var(--link)!important;text-decoration:underline;text-underline-offset:4px}.ah-library a:hover{color:#8f4f45!important}.ah-library :focus-visible{outline:3px solid #8f4f45;outline-offset:4px}.ah-library nav{display:flex;gap:16px 28px;flex-wrap:wrap;margin:28px 0 48px}.ah-library section{scroll-margin-top:140px}.ah-library .pathway{display:grid;grid-template-columns:1.1fr 1fr;gap:36px;padding:40px 0;border-top:1px solid #bac8bb}.ah-library .pathway ul{margin:0;padding-left:22px}.ah-library .pathway li{margin:0 0 12px}.ah-library .pathway .paid{margin-top:24px}.ah-library .access{font-size:13px;font-weight:600}.ah-library .browse{margin-top:48px;border-top:1px solid #bac8bb;padding-top:40px}.ah-library .filters{display:flex;flex-wrap:wrap;gap:16px;margin:24px 0}.ah-library label{display:flex;flex-direction:column;gap:6px;font-size:14px;font-weight:600;flex:1;min-width:150px}.ah-library input,.ah-library select{font:inherit;color:var(--ink);background:#fff;border:1px solid #809784;border-radius:3px;padding:12px;min-height:46px;width:100%}.ah-library .catalog-list{list-style:none;padding:0;margin:16px 0}.ah-library .catalog-list li{display:flex;justify-content:space-between;gap:20px;padding:16px 0;border-top:1px solid #cbd5cb;align-items:baseline}.ah-library .catalog-list li[hidden]{display:none}.ah-library .catalog-list span{flex-shrink:0;font-size:13px}.ah-library .subscribe{margin-top:44px;padding-top:28px;border-top:1px solid #bac8bb}.ah-library .empty{font-weight:600}.ah-library button{font:inherit;color:#fff;background:#2e6b46;border:0;border-radius:3px;padding:12px 18px;cursor:pointer}.ah-library [hidden]{display:none!important}@media(max-width:640px){.ah-library{margin:24px auto}.ah-library .pathway{grid-template-columns:1fr;gap:16px}.ah-library .catalog-list li{display:block}.ah-library .catalog-list span{display:block;margin-top:4px}.ah-library nav{display:block}.ah-library nav a{display:block;margin:12px 0}}\n<\/style><div class=\"ah-library\" id=\"ah-product-library\"><h2 class=\"library-heading\">Find your next garden resource<\/h2><p>Free printables, planning tools and paid guides for California gardeners. Choose a starting path or browse the collection below.<\/p><nav aria-label=\"Garden resource paths\"><a href=\"#library-family\" data-library-asset=\"\">Garden together<\/a><a href=\"#library-seasonal\" data-library-asset=\"\">Plan your gardening year<\/a><a href=\"#library-firewise\" data-library-asset=\"\">Plan a fire-wise garden<\/a><a href=\"#library-catalog\" data-library-asset=\"\">Browse all resources<\/a><\/nav><section class=\"pathway\" id=\"library-family\" aria-labelledby=\"title-family\"><div><h2 id=\"title-family\">Garden together<\/h2><p>Find a project for your family, then keep the observations, drawings and discoveries in one place.<\/p><p class=\"access\">Free starting resource<\/p><p><a href=\"/your-garden-toolkit#download=garden-checklist\" data-library-asset=\"free:garden-checklist\">Beginner Garden Setup Checklist<\/a><\/p><ul><li><a href=\"/planting-calendar\" data-library-asset=\"\">Open the interactive planting calendar<\/a><\/li><\/ul><div class=\"paid\"><p class=\"access\">Paid guide · price and download details in the shop<\/p><h3><a href=\"/store/p/kids-garden-activity-pack-santa-cruz-county-edition\" data-library-asset=\"paid:kids-garden-activity-pack-santa-cruz-county-edition\">Kids Garden Activity Pack - Santa Cruz County Edition<\/a><\/h3><p>The Kids’ Garden Activity Pack brings observation pages, scavenger hunts, growing projects, crafts and a seasonal section together for ages 5–12, with parent guidance.<\/p><\/div><\/div><div><h3>Read and put it to use<\/h3><ul><li><a href=\"https://www.ambitiousharvest.com/learn/gardening-with-preschoolers\" data-library-asset=\"\">Gardening with preschoolers<\/a><\/li><li><a href=\"https://www.ambitiousharvest.com/learn/gardening-with-toddlers-ages-2-4\" data-library-asset=\"\">Gardening with toddlers<\/a><\/li><li><a href=\"https://www.ambitiousharvest.com/learn/gardening-projects-elementary-kids\" data-library-asset=\"\">Projects for elementary-age children<\/a><\/li><\/ul><p><a href=\"https://www.ambitiousharvest.com/your-garden-toolkit#ah-toolkit-form\" data-library-asset=\"\">Get garden resources by email<\/a><\/p><\/div><\/section><section class=\"pathway\" id=\"library-seasonal\" aria-labelledby=\"title-seasonal\"><div><h2 id=\"title-seasonal\">Plan your gardening year<\/h2><p>Use the calendar to choose your next planting window, a checklist to organize the work, and a journal to keep what you learn.<\/p><p class=\"access\">Free starting resource<\/p><p><a href=\"/your-garden-toolkit#download=seasonal-planting-calendar\" data-library-asset=\"free:seasonal-planting-calendar\">Seasonal Planting Calendar<\/a><\/p><ul><li><a href=\"/planting-calendar\" data-library-asset=\"\">Open the interactive planting calendar<\/a><\/li><\/ul><div class=\"paid\"><p class=\"access\">Paid guide · price and download details in the shop<\/p><h3><a href=\"/store/p/seasonal-planting-master-guide-santa-cruz-county-edition\" data-library-asset=\"paid:seasonal-planting-master-guide-santa-cruz-county-edition\">Seasonal Planting Master Guide - Santa Cruz County Edition<\/a><\/h3><p>The Seasonal Planting Master Guide organizes the Santa Cruz County gardening year into twelve monthly chapters, with planning worksheets. The Garden Planner &amp; Journal is a separate place to keep your records.<\/p><\/div><\/div><div><h3>Read and put it to use<\/h3><ul><li><a href=\"https://www.ambitiousharvest.com/learn/brassica-planting-calendar-santa-cruz\" data-library-asset=\"\">Plan brassica planting<\/a><\/li><li><a href=\"https://www.ambitiousharvest.com/learn/santa-cruz-microclimates\" data-library-asset=\"\">Understand your microclimate<\/a><\/li><\/ul><p><a href=\"https://www.ambitiousharvest.com/your-garden-toolkit#ah-toolkit-form\" data-library-asset=\"\">Get garden resources by email<\/a><\/p><\/div><\/section><section class=\"pathway\" id=\"library-firewise\" aria-labelledby=\"title-firewise\"><div><h2 id=\"title-firewise\">Plan a fire-wise garden<\/h2><p>Start with the free guide, then explore the existing planning and maintenance resources for a California food garden.<\/p><p class=\"access\">Free starting resource<\/p><p><a href=\"/your-garden-toolkit#download=fire-wise-gardening-guide\" data-library-asset=\"free:fire-wise-gardening-guide\">Fire-Wise Gardening Guide<\/a><\/p><div class=\"paid\"><p class=\"access\">Paid guide · price and download details in the shop<\/p><h3><a href=\"/store/p/firewise-food-garden-kit-california-edition\" data-library-asset=\"paid:firewise-food-garden-kit-california-edition\">Firewise Food Garden Kit - California Edition<\/a><\/h3><p>The Firewise Food Garden Kit adds assessment, layout, maintenance and recordkeeping materials to the introductory guide. Use current local fire-agency guidance for requirements at your property.<\/p><\/div><\/div><div><h3>Read and put it to use<\/h3><ul><li><a href=\"https://www.ambitiousharvest.com/learn/fire-wise-raised-bed-materials-and-placement\" data-library-asset=\"\">Raised-bed materials and placement<\/a><\/li><li><a href=\"https://www.ambitiousharvest.com/learn/preparing-your-vegetable-garden-for-fire-season\" data-library-asset=\"\">Prepare your vegetable garden for fire season<\/a><\/li><li><a href=\"https://www.ambitiousharvest.com/learn/fire-wise-maintenance-a-seasonal-checklist-for-santa-cruz-gardeners\" data-library-asset=\"\">Seasonal maintenance checklist<\/a><\/li><\/ul><p><a href=\"https://www.ambitiousharvest.com/your-garden-toolkit#ah-toolkit-form\" data-library-asset=\"\">Get garden resources by email<\/a><\/p><\/div><\/section><section class=\"browse\" id=\"library-catalog\"><h2>Browse the collection<\/h2><div class=\"filters\" hidden><label>Search resources<input type=\"search\" id=\"library-search\" placeholder=\"Try seeds, berries or a planner\"><\/label><label>Show<select id=\"library-access\"><option value=\"all\">Everything<\/option><option value=\"free\">Free resources<\/option><option value=\"paid\">Paid guides and services<\/option><\/select><\/label><\/div><p id=\"library-count\" role=\"status\" aria-live=\"polite\">44 resources<\/p><ul class=\"catalog-list\"><li data-search=\"california berry growing cheat sheet\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=berry-growing-cheat-sheet\" data-library-asset=\"free:berry-growing-cheat-sheet\">California Berry Growing Cheat Sheet<\/a><span>Free PDF<\/span><\/li><li data-search=\"companion planting guide\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=companion-planting\" data-library-asset=\"free:companion-planting\">Companion Planting Guide<\/a><span>Free PDF<\/span><\/li><li data-search=\"fire-wise gardening guide\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=fire-wise-gardening-guide\" data-library-asset=\"free:fire-wise-gardening-guide\">Fire-Wise Gardening Guide<\/a><span>Free PDF<\/span><\/li><li data-search=\"california fruit tree planting guide\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=fruit-tree-planting-guide\" data-library-asset=\"free:fruit-tree-planting-guide\">California Fruit Tree Planting Guide<\/a><span>Free PDF<\/span><\/li><li data-search=\"beginner garden setup checklist\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=garden-checklist\" data-library-asset=\"free:garden-checklist\">Beginner Garden Setup Checklist<\/a><span>Free PDF<\/span><\/li><li data-search=\"garden troubleshooting guide\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=garden-troubleshooting\" data-library-asset=\"free:garden-troubleshooting\">Garden Troubleshooting Guide<\/a><span>Free PDF<\/span><\/li><li data-search=\"gopher control guide\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=gopher-guide\" data-library-asset=\"free:gopher-guide\">Gopher Control Guide<\/a><span>Free PDF<\/span><\/li><li data-search=\"know your microclimate worksheet\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=microclimate-worksheet\" data-library-asset=\"free:microclimate-worksheet\">Know Your Microclimate Worksheet<\/a><span>Free PDF<\/span><\/li><li data-search=\"california native and drought-tolerant plant cheat sheet\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=native-drought-cheat-sheet\" data-library-asset=\"free:native-drought-cheat-sheet\">California Native and Drought-Tolerant Plant Cheat Sheet<\/a><span>Free PDF<\/span><\/li><li data-search=\"california pepper growing cheat sheet\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=pepper-growing-cheat-sheet\" data-library-asset=\"free:pepper-growing-cheat-sheet\">California Pepper Growing Cheat Sheet<\/a><span>Free PDF<\/span><\/li><li data-search=\"predator-proof coop checklist\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=predator-proof-coop-checklist\" data-library-asset=\"free:predator-proof-coop-checklist\">Predator-Proof Coop Checklist<\/a><span>Free PDF<\/span><\/li><li data-search=\"seasonal planting calendar\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=seasonal-planting-calendar\" data-library-asset=\"free:seasonal-planting-calendar\">Seasonal Planting Calendar<\/a><span>Free PDF<\/span><\/li><li data-search=\"seasonal planting calendar: monthly reference\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=seasonalplantingcalendar\" data-library-asset=\"free:seasonalplantingcalendar\">Seasonal Planting Calendar: Monthly Reference<\/a><span>Free PDF<\/span><\/li><li data-search=\"seasonal garden tasks checklist\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=seasonaltaskschecklist\" data-library-asset=\"free:seasonaltaskschecklist\">Seasonal Garden Tasks Checklist<\/a><span>Free PDF<\/span><\/li><li data-search=\"california seed starting cheat sheet\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=seed-starting-cheat-sheet\" data-library-asset=\"free:seed-starting-cheat-sheet\">California Seed Starting Cheat Sheet<\/a><span>Free PDF<\/span><\/li><li data-search=\"seed starting guide\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=seed-starting-guide\" data-library-asset=\"free:seed-starting-guide\">Seed Starting Guide<\/a><span>Free PDF<\/span><\/li><li data-search=\"tomato variety selector\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=tomato-selector\" data-library-asset=\"free:tomato-selector\">Tomato Variety Selector<\/a><span>Free PDF<\/span><\/li><li data-search=\"vegetables by season\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=vegetables-by-season\" data-library-asset=\"free:vegetables-by-season\">Vegetables by Season<\/a><span>Free PDF<\/span><\/li><li data-search=\"water-wise gardening guide\" data-access=\"free\" data-kind=\"pdf\"><a href=\"/your-garden-toolkit#download=water-wise-gardening-guide\" data-library-asset=\"free:water-wise-gardening-guide\">Water-Wise Gardening Guide<\/a><span>Free PDF<\/span><\/li><li data-search=\"the california berry growing guide\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/california-berry-growing-guide\" data-library-asset=\"paid:california-berry-growing-guide\">The California Berry Growing Guide<\/a><span>Paid PDF<\/span><\/li><li data-search=\"california dahlia growing guide\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/california-dahlia-growing-guide\" data-library-asset=\"paid:california-dahlia-growing-guide\">California Dahlia Growing Guide<\/a><span>Paid PDF<\/span><\/li><li data-search=\"companion planting master chart &amp; guide\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/companion-planting-master-chart-guide\" data-library-asset=\"paid:companion-planting-master-chart-guide\">Companion Planting Master Chart &amp; Guide<\/a><span>Paid PDF<\/span><\/li><li data-search=\"composting &amp; soil building guide\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/composting-soil-building-guide\" data-library-asset=\"paid:composting-soil-building-guide\">Composting &amp; Soil Building Guide<\/a><span>Paid PDF<\/span><\/li><li data-search=\"container &amp; small space garden guide\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/container-small-space-garden-guide\" data-library-asset=\"paid:container-small-space-garden-guide\">Container &amp; Small Space Garden Guide<\/a><span>Paid PDF<\/span><\/li><li data-search=\"firewise food garden kit - california edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/firewise-food-garden-kit-california-edition\" data-library-asset=\"paid:firewise-food-garden-kit-california-edition\">Firewise Food Garden Kit - California Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"first harvest kit - california edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/first-harvest-kit-california-edition\" data-library-asset=\"paid:first-harvest-kit-california-edition\">First Harvest Kit - California Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"garden planner &amp; journal - santa cruz county edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/garden-planner-journal-santa-cruz-county-edition\" data-library-asset=\"paid:garden-planner-journal-santa-cruz-county-edition\">Garden Planner &amp; Journal - Santa Cruz County Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"gopher &amp; pest defense kit - santa cruz county edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/gopher-pest-defense-kit-santa-cruz-county-edition\" data-library-asset=\"paid:gopher-pest-defense-kit-santa-cruz-county-edition\">Gopher &amp; Pest Defense Kit - Santa Cruz County Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"herb growing kitchen garden guide - california edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/herb-growing-kitchen-garden-guide-california-edition\" data-library-asset=\"paid:herb-growing-kitchen-garden-guide-california-edition\">Herb Growing Kitchen Garden Guide - California Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"kids garden activity pack - santa cruz county edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/kids-garden-activity-pack-santa-cruz-county-edition\" data-library-asset=\"paid:kids-garden-activity-pack-santa-cruz-county-edition\">Kids Garden Activity Pack - Santa Cruz County Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"microclimate mastery guide - santa cruz county edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/microclimate-mastery-guide-santa-cruz-county-edition\" data-library-asset=\"paid:microclimate-mastery-guide-santa-cruz-county-edition\">Microclimate Mastery Guide - Santa Cruz County Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"preserving the harvest guide - california edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/preserving-the-harvest-guide-california-edition\" data-library-asset=\"paid:preserving-the-harvest-guide-california-edition\">Preserving the Harvest Guide - California Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"seasonal planting master guide - santa cruz county edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/seasonal-planting-master-guide-santa-cruz-county-edition\" data-library-asset=\"paid:seasonal-planting-master-guide-santa-cruz-county-edition\">Seasonal Planting Master Guide - Santa Cruz County Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"seed starting success kit - santa cruz county edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/seed-starting-success-kit-santa-cruz-county-edition\" data-library-asset=\"paid:seed-starting-success-kit-santa-cruz-county-edition\">Seed Starting Success Kit - Santa Cruz County Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"the california backyard flock starter guide\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/the-california-backyard-flock-starter-guide\" data-library-asset=\"paid:the-california-backyard-flock-starter-guide\">The California Backyard Flock Starter Guide<\/a><span>Paid PDF<\/span><\/li><li data-search=\"the tomato growing masterkit - california edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/tomato-growing-masterkit-california-edition\" data-library-asset=\"paid:tomato-growing-masterkit-california-edition\">The Tomato Growing MasterKit - California Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"water-wise garden workbook - california edition\" data-access=\"paid\" data-kind=\"pdf\"><a href=\"/store/p/water-wise-garden-workbook-california-edition\" data-library-asset=\"paid:water-wise-garden-workbook-california-edition\">Water-Wise Garden Workbook - California Edition<\/a><span>Paid PDF<\/span><\/li><li data-search=\"virtual garden design\" data-access=\"paid\" data-kind=\"service\"><a href=\"/store/p/virtual-garden-design\" data-library-asset=\"service:virtual-garden-design\">Virtual Garden Design<\/a><span>Paid service<\/span><\/li><li data-search=\"virtual garden review\" data-access=\"paid\" data-kind=\"service\"><a href=\"/store/p/virtual-garden-review\" data-library-asset=\"service:virtual-garden-review\">Virtual Garden Review<\/a><span>Paid service<\/span><\/li><li data-search=\"build your flock\" data-access=\"free\" data-kind=\"tool\"><a href=\"/build-your-flock\" data-library-asset=\"tool:build-your-flock\">Build Your Flock<\/a><span>Free tool<\/span><\/li><li data-search=\"garden conditions\" data-access=\"free\" data-kind=\"tool\"><a href=\"/garden-conditions\" data-library-asset=\"tool:garden-conditions\">Garden Conditions<\/a><span>Free tool<\/span><\/li><li data-search=\"local resources\" data-access=\"free\" data-kind=\"tool\"><a href=\"/local-resources\" data-library-asset=\"tool:local-resources\">Local Resources<\/a><span>Free tool<\/span><\/li><li data-search=\"planting calendar\" data-access=\"free\" data-kind=\"tool\"><a href=\"/planting-calendar\" data-library-asset=\"tool:planting-calendar\">Planting Calendar<\/a><span>Free tool<\/span><\/li><li data-search=\"tomato quiz\" data-access=\"free\" data-kind=\"tool\"><a href=\"/tomato-quiz\" data-library-asset=\"tool:tomato-quiz\">Tomato Quiz<\/a><span>Free tool<\/span><\/li><\/ul><p class=\"empty\" hidden>No matching resources. Try a broader search.<\/p><\/section><section class=\"subscribe\"><h2>Keep the resources handy<\/h2><p>The free PDFs open through Your Garden Toolkit. If you would also like garden resources by email, use the existing signup form.<\/p><p><a href=\"https://www.ambitiousharvest.com/your-garden-toolkit#ah-toolkit-form\" data-library-asset=\"\">Go to email signup<\/a><\/p><\/section><\/div>", "routes": {"https://www.ambitiousharvest.com/learn/gardening-with-preschoolers": "#library-family", "https://www.ambitiousharvest.com/learn/gardening-with-toddlers-ages-2-4": "#library-family", "https://www.ambitiousharvest.com/learn/gardening-projects-elementary-kids": "#library-family", "https://www.ambitiousharvest.com/learn/brassica-planting-calendar-santa-cruz": "#library-seasonal", "https://www.ambitiousharvest.com/learn/santa-cruz-microclimates": "#library-seasonal", "https://www.ambitiousharvest.com/learn/fire-wise-raised-bed-materials-and-placement": "#library-firewise", "https://www.ambitiousharvest.com/learn/preparing-your-vegetable-garden-for-fire-season": "#library-firewise", "https://www.ambitiousharvest.com/learn/fire-wise-maintenance-a-seasonal-checklist-for-santa-cruz-gardeners": "#library-firewise"}, "version": "33ff9d576135"};
  function run(){
    var path=location.pathname.replace(/\/$/,'')||'/';
    var root=document.getElementById('ah-product-library');
    if(path==='/your-garden-toolkit'&&!root){
      var main=document.querySelector('main')||document.getElementById('page');
      if(!main)return;
      var host=document.createElement('div');host.id='ah-library-host';host.innerHTML=config.fragment;
      var header=document.querySelector('header');host.style.paddingTop=((header?header.getBoundingClientRect().height:100)+24)+'px';main.insertBefore(host,main.firstChild);root=document.getElementById('ah-product-library');
    }
    if(root&&!root.dataset.bound){
      root.dataset.bound=config.version;
      var filters=root.querySelector('.filters');if(filters)filters.hidden=false;
      var search=root.querySelector('#library-search'),access=root.querySelector('#library-access');
      function filter(){var n=0,term=search.value.trim().toLowerCase();root.querySelectorAll('.catalog-list li').forEach(function(row){row.hidden=!(row.dataset.search.indexOf(term)>=0&&(access.value==='all'||access.value===row.dataset.access));if(!row.hidden)n++;});root.querySelector('#library-count').textContent=n+' matching resources';root.querySelector('.empty').hidden=n!==0;}
      if(search&&access){search.addEventListener('input',filter);access.addEventListener('change',filter);}
      root.addEventListener('click',function(e){var a=e.target.closest('a');if(!a||!e.isTrusted)return;if(typeof window.gtag==='function')window.gtag('event','library_resource_click',{asset_id:a.dataset.libraryAsset||'pathway',destination_path:new URL(a.href,location.href).pathname,library_version:config.version});});
      function scroll(){if(/^#library-(family|seasonal|firewise|catalog)$/.test(location.hash)||location.hash==='#ah-product-library'){var target=document.getElementById(location.hash.slice(1));if(target)target.scrollIntoView({block:'start'});}}
      window.addEventListener('hashchange',scroll);scroll();
      if(path==='/your-garden-toolkit'&&!document.getElementById('ah-library-schema')){
        var schema=document.createElement('script');schema.type='application/ld+json';schema.id='ah-library-schema';
        schema.textContent=JSON.stringify({'@context':'https://schema.org','@type':'ItemList',name:'Ambitious Harvest Garden Resources',itemListElement:Array.from(root.querySelectorAll('.catalog-list a')).map(function(a,i){return {'@type':'ListItem',position:i+1,name:a.textContent,url:new URL(a.getAttribute('href'),'https://www.ambitiousharvest.com').href};})});document.head.appendChild(schema);
      }
    }
    var destination=config.routes['https://www.ambitiousharvest.com'+path];
    if(destination&&!document.getElementById('ah-library-article-link')){
      var body=document.querySelector('.blog-item-content-wrapper')||document.querySelector('[data-content-field="body"]')||document.querySelector('.entry-content');
      if(body){var p=document.createElement('p');p.id='ah-library-article-link';var a=document.createElement('a');a.href='/your-garden-toolkit'+destination;a.textContent='Explore the related free resources, tools and paid guides';p.appendChild(a);body.appendChild(p);}
    }
  }
